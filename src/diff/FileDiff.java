...
public class FileDiff
{
    ...
    private static List<String> whiteList; // 白名单 , 哪些差异允许显示
    ...
    protected void printLines(int start, int end, String ind, String[] lines)
    {
        for (int lnum = start; lnum <= end; ++lnum) {
            int finalLnum = lnum;
            whiteList.stream().forEach(t->{
                if (lines[finalLnum].indexOf(t)>0){
                    System.out.println(ind + " " + lines[finalLnum]);
                }
            });
        }
    }
...
//        }
        ...
        URL url1 = FileDiff.class.getResource("/diff/json1.txt");
        URL url2 = FileDiff.class.getResource("/diff/json2.txt");
        URL url3 = FileDiff.class.getResource("/diff/json3.txt");

        FileDiff.whiteList = new ArrayList<>();
        whiteList.add("retcode");
        whiteList.add("retmsg");
//        whiteList.add(":");

    	new FileDiff(url1.getPath(), url3.getPath());
    }
